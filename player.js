...
const NEGAMAX_CTRL_STOP_TIME        = 0x04;                         //  (See C++ code) Set this byte in commandFlags to indicate that search is timed.
...
class Player
  {
    ...
    constructor()
      {
        ...
        this.searchId = 0;                                          //  Monotonic branch/search identifier.
        ...
        this.negamaxStatisticsBuffer = null;                        //  ByteArray representation of the history-heuristic table.
                                        ...
                                        for(i = 1; i < parse.length; i++)
                                          {
                                            ...
                                            this.parent.ZobristHashBuffer[i - 1] = arr[0];
                                          }
    ...
    /* Advance Philadelphia while it is Philadelphia's turn. */
    stepMyTurn()
      {
        var branch;
        var status;
        ...
        switch(branch.status)
          {
              ...
              if(status == NEGAMAX_STATUS_STOP_TIME)
                {
                  if(this.hasUsableMove(branch))
                    ...
                    console.error('Philadelphia: time stop occurred before any usable move was completed.');
                }
              break;
            ...
            default:
              console.error('Philadelphia: unknown branch status on AI turn: ' + branch.status);
              break;
          }
    ...
    /* Advance Philadelphia while the opponent is thinking. */
    stepPondering()
      {
        var branch;
        ...
        switch(branch.status)
          {
            ...
            default:
              console.error('Philadelphia: unknown branch status while pondering: ' + branch.status);
              this.advancePonderingBranch();
              break;
          }
    ...
    stopObsoleteSearch()
      {
        ...
        if(status == NEGAMAX_STATUS_DONE)
          {
            if(activeIndex >= 0)
              ...
              {
                console.warn('Philadelphia: Negamax is DONE but its search ID matches no retained branch.');
                this.collectNodes(-1);
              }
            return;
          }
        ...
        if(status == NEGAMAX_STATUS_STOP_REQUESTED ||
          ...
          console.error('Philadelphia: unexpected Negamax status while stopping obsolete search: ' + status);
        ...
        switch(status)
          {
            ...
            case NEGAMAX_STATUS_DONE:
              if(activeIndex >= 0)
                ...
                {
                  console.error('Philadelphia: completed Negamax result has no matching branch ID.');
                  this.collectNodes(-1);
                }
              break;
            ...
            case NEGAMAX_STATUS_IDLE:
              console.error('Philadelphia: branch is marked SEARCHING while Negamax is IDLE.');
              if(branchIndex >= 0 && branchIndex < this.branches.length)
                this.branches[branchIndex].status = STATUS_DONE_SEARCH;
              break;

            default:
              console.error('Philadelphia: unknown Negamax status: ' + status);
              break;
          }
    ...
    harvestCompletedSearch(branchIndex)
      {
        ...
        if(this.negamaxEngine.instance.exports.getSearchId() != branch.id)
          {
            console.error('Philadelphia: refusing stale Negamax result (search ID mismatch).');
        ...
        for(i = 0; i < _GAMESTATE_BYTE_SIZE; i++)
          if(this.negamaxOutputBuffer[i] != branch.gamestate[i])
            {
              console.error('Philadelphia: refusing stale Negamax result (root game-state mismatch).');
              this.collectNodes(branchIndex);
              branch.status = STATUS_DONE_SEARCH;
              return false;
            }
        ...
        if(depthAchieved != targetDepth)
          {
            console.error('Philadelphia: completed search reports depth ' + depthAchieved +
    ...
    lookup(branchIndex = this.branchIterator)
      {
        ...
        params = 'sendRequest=philadelphia';
        for(i = 0; i < _GAMESTATE_BYTE_SIZE; i++)
        ...
        request.onreadystatechange = () =>
          {
            ...
            if(parse[0] != 'chess' || parse[1] != 'found')
              return;

            parse = parse[2].split(',');
            if(parse.length < _MOVE_BYTE_SIZE)
              {
                console.warn('Philadelphia: malformed opening-book move.');
                return;
              }
    ...
    playBestMove(branch)
      {
        if(!this.hasUsableMove(branch))
          {
            console.error('Philadelphia: refusing to play an unset best move.');
            return;
          }
